...
update() {
 ...
 if [ "$NEW_VERSION" != "$OLD_VERSION" ] && [ -n "$NEW_VERSION" ] && [ -n "$OLD_VERSION" ]; then # if both variables are defined and not the same
  ...
  echo "You will now be prompted for your password to install this update. To cancel, hit cancel or press Ctrl+C."
  pkexec --keep-cwd dpkg -i discord_latest.deb || exit 1 # install and fail if the install fails
  echo "Cleaning up..."
